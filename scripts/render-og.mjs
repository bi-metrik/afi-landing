#!/usr/bin/env node
/**
 * Renderiza assets/og/afi-og.png (1200x630) desde scripts/og-fuente.html.
 *
 * Es la imagen que ven WhatsApp y LinkedIn al pegar el enlace. Si cambia el
 * titular del hero, cambia el del artboard y se vuelve a correr esto:
 *
...
const PORT = 9337
const PROFILE = `${process.env.TMPDIR || '/tmp'}/afi-og-profile`
const SALIDA = resolve(DIR, 'assets/og/afi-og.png')
...
try {
  ...
  console.log(`afi-og.png  ${caja.w}x${caja.h}`)
} finally {
  ws?.close()
  chrome.kill()
}
